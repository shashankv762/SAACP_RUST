...
/// every `.advance(...)` call site auto-derefs through `LazyLock` → `Arc`
/// unchanged.
pub static GLOBAL_IDENTITY_GATE: LazyLock<std::sync::Arc<IdentityGate>> =
    LazyLock::new(|| std::sync::Arc::new(IdentityGate::new()));
