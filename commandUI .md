...
/* fx */
.flashfx{position:fixed;inset:0;z-index:80;pointer-events:none;
  background:radial-gradient(ellipse at center, transparent 38%, rgba(255,107,71,.55) 100%);
  animation:vignette 1.5s ease-out forwards}
@keyframes vignette{0%{opacity:0}10%{opacity:1}100%{opacity:0}}
...
function initFin() {
  ...
  return {
    saved: 432.10, tokensSaved: 21605000, blocks: 9,
    earned: [], hours, velHist: [],
    txs: [
      { id: 'TXN-' + hexN(4).toUpperCase(), time: '09:12:44', agent: 'Agent-X', gate: 'Gate 4.0', tokens: 1240000, usd: 24.80 },
      { id: 'TXN-' + hexN(4).toUpperCase(), time: '10:03:18', agent: 'Ad-Optimizer', gate: 'Gate 12.0', tokens: 4910000, usd: 98.20 },
      { id: 'TXN-' + hexN(4).toUpperCase(), time: '11:47:02', agent: 'Malicious-Agent-X', gate: 'Gate 4.0', tokens: 2130000, usd: 42.60 },
    ],
  };
}
...
export default function App() {
  ...
  const triggerInjection = useCallback(() => {
    ...
    setShake(true);
    const stT = setTimeout(() => setShake(false), 520); timersRef.current.push(stT);
  ...
  const resetMesh = useCallback(() => {
    ...
    setFocusId(null);
    syncSnapshot();
    pushFeed('system', 'RESET', 'info', 'Mesh topology reset', ['10 agents re-attested · genesis block replayed']);
  ...
  const reloadConfig = useCallback(() => {
    const to = setTimeout(() => {
      toast('saacp.toml hot-reloaded · 12 gates re-armed', 'ok');
      pushFeed('system', 'CONFIG', 'ok', 'Configuration hot-reload complete',
        ['source: /etc/saacp/saacp.toml · gate policies re-verified · 0 violations']);
    }, 700);
    timersRef.current.push(to);
    return to;
  }, [pushFeed, toast]);
  ...
  useEffect(() => {
    const w = worldRef.current;
    setAgents([...w.agents.values()].map((a) => ({ ...a })));
    pushFeed('system', 'BOOT', 'info', 'SAACP Gateway v0.1-beta2 online', ['12 gates armed · ring 0 · policy epoch 7 · non-reorderable']);
    pushFeed('system', 'AUDIT', 'ok', 'ImmutableAuditLog mounted', [`genesis 0x${hexN(4)}…${hexN(4)} · hash-chained · append-only`]);
    pushFeed('system', 'SSE', 'info', '/events stream listening', ['axum 0.7 · command_center.rs · read-only projection']);
    syncSnapshot();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  /* LIVE mode → attempt SSE, fall back to simulation */
  useEffect(() => {
    if (mode !== 'live') { setSseOk(false); return; }
    let es = null, to = null, settled = false;
    const fail = () => {
      if (settled) return; settled = true;
      toast('Gateway unreachable at /events — engaging Simulation Mode', 'warn');
      pushFeed('system', 'SYSTEM', 'warn', 'SSE handshake failed', ['target: http://localhost:9090/events · fallback: client-side simulator']);
      setMode('sim');
    };
    to = setTimeout(fail, 2600);
    try {
      es = new EventSource('/events');
      es.onopen = () => {
        if (settled) return; settled = true;
        clearTimeout(to); setSseOk(true);
      ...
      es.onmessage = (ev) => {
        try {
          ...
          const w = worldRef.current;
          if (j.type === 'TrustSignal' && j.agent_id && w.agents.get(j.agent_id)) {
            const a = w.agents.get(j.agent_id);
            if (typeof j.score === 'number') { a.trust = clamp(j.score, 0, 1); refreshStatus(a); }
          } else if (j.type === 'DelegationEdge' && j.source && j.target) {
            w.pulses.push({ s: j.source, d: j.target, t: 0, speed: 1 / 900, color: '#c5f547', r: 2.4 });
          } else if (j.type === 'InjectionAlert') {
            pushFeed('attack', 'BLOCKED', 'crit', 'Gate 4.0 intercept (live gateway)', [j.summary || 'see audit log']);
            lastAttackRef.current = Date.now();
          } else if (j.type === 'AuditEntry') {
            pushFeed('delegation', 'AUDIT', 'info', `${j.source || '?'} → ${j.target || '?'}`, [j.intent_prefix || '']);
          }
          syncSnapshot();
        } catch { /* malformed frame — ignore */ }
      };
      es.onerror = () => { clearTimeout(to); try { es.close(); } catch { /* noop */ } fail(); };
    } catch { fail(); }
    return () => { clearTimeout(to); try { es && es.close(); } catch { /* noop */ } };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [mode]);
  ...
  return (
    <div className={`saacp ${shake ? 'shaking' : ''}`}>
      ...
      <div className="grain" />
      {flashKey > 0 && <div className="flashfx" key={flashKey} />}
      ...
      <motion.div className="ctl panel" initial={{ y: 90, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.35, type: 'spring', stiffness: 120 }}>
        ...
        <button className="btn btn-red" onClick={triggerInjection}>☣ TRIGGER PROMPT INJECTION</button>
        <button className="btn btn-amber" onClick={triggerLoop} disabled={loopActive}>
...
function AuditChip() {
  ...
  useEffect(() => { const iv = setInterval(() => setH(hexN(8)), 4000); return () => clearInterval(iv); }, []);
  return (
