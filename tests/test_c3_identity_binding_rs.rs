...
use saacp::{
    AgentIdentityCertificate, IdentityGate, IdentityVerifier, SAACPBytecodes,
    SessionIdentityRegistry, TranscriptBoundSession, IDENTITY_GATE_PHASES,
};
...
fn test_certificate_body_bytes_not_empty() {
    ...
    let body = cert
        .body_bytes()
        .expect("body_bytes must succeed for a freshly-issued certificate");
...
fn test_verifier_require_identity_verified_fails_if_not_done() {
    let verifier = IdentityVerifier::new();
    assert!(verifier
        .require_identity_verified("nobody", "cap_check")
        .is_err());
}
...
fn test_verify_transcript_binding_key_swap_detected() {
    ...
    let verifier = IdentityVerifier::new();
    let err = verifier
        .verify_transcript_binding(&session, "client-agent", &"ff".repeat(32), "client")
        .unwrap_err();
...
fn test_verify_transcript_binding_identity_splice_detected() {
    ...
    let verifier = IdentityVerifier::new();
    let err = verifier
        .verify_transcript_binding(&session, "impostor-agent", &"aa".repeat(32), "client")
        .unwrap_err();
...
fn test_verify_thash_mismatch_detected() {
    ...
    let verifier = IdentityVerifier::new();
    let err = verifier
        .verify_thash_matches_capability(&session, "wrong_hash_value")
        .unwrap_err();
...
fn test_identity_gate_require_phase_fails_before_advance() {
    ...
    assert!(gate
        .require_phase("agent-1", "session-1", "IDENTITY_VERIFIED")
        .is_err());
}
...
fn test_identity_gate_advance_and_require_ok() {
    let gate = IdentityGate::new();
    gate.advance("agent-1", "session-1", "IDENTITY_VERIFIED")
        .unwrap();
    assert!(gate
        .require_phase("agent-1", "session-1", "IDENTITY_VERIFIED")
        .is_ok());
...
fn test_identity_gate_ordered_phases() {
    ...
    assert!(gate
        .require_phase(agent, sid, "CAPABILITY_VALIDATED")
        .is_err());
}
...
fn test_identity_gate_independent_per_agent_session() {
    let gate = IdentityGate::new();
    gate.advance("agent-A", "sess-A", "IDENTITY_VERIFIED")
        .unwrap();
    // agent-B has not advanced yet
    assert!(gate
        .require_phase("agent-B", "sess-A", "IDENTITY_VERIFIED")
        .is_err());
}
...
fn test_session_registry_get_nonexistent_returns_none() {
    let reg = SessionIdentityRegistry::new();
    assert!(reg
        .get_by_thash("nonexistent", |s| s.client_agent_id.clone())
        .is_none());
}
