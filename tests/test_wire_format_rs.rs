...
use saacp::{
    EasiEncryptor, MEASCFrame, SessionEpochManager, MEASC_AUTH_TAG_SIZE,
    MEASC_CONTEXT_REF_ID_OFFSET, MEASC_CONTEXT_REF_ID_SIZE, MEASC_DEFAULT_EPOCH_PACKET_THRESHOLD,
    MEASC_DEFAULT_EPOCH_TIME_SECONDS, MEASC_HEADER_SIZE, MEASC_MAGIC,
};
...
fn test_frame_epoch_id_at_offset_32() {
    ...
    assert_eq!(
        epoch_id, 0,
        "epoch_id at [32..36] must be 0 for first epoch"
    );
}
...
fn test_frame_has_auth_tag_after_header() {
    ...
    assert!(
        frame.len() >= MEASC_HEADER_SIZE + MEASC_AUTH_TAG_SIZE,
...
fn test_larger_payload_produces_larger_frame() {
    ...
    assert!(
        frame_long.len() > frame_short.len(),
...
fn test_easi_different_ctx_refs_produce_different_wire_bytes() {
    ...
    let make_frame_with_ctx = |ctx: [u8; 32]| -> Vec<u8> {
        ...
        mgr.with_epoch_mut(&sid, 0, |epoch| {
            MEASCFrame::build_frame(
                epoch, 0x01, 0x00, 0x00, 0x00, b"payload", &ctx, &[0u8; 24], 0,
            )
        })
        .unwrap()
        .unwrap()
...
fn test_easi_roundtrip_context_ref_id_recovered() {
    ...
    .unwrap();
    let parsed = MEASCFrame::parse_frame(&frame, &mgr2, true).expect("parse_frame must succeed");

    assert_eq!(
        parsed.context_ref_id, ctx_ref_plaintext,
