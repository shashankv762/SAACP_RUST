...
- `gateway.rs::AgentRateLimiter` — a genuinely new hybrid shape, not the plain either/or wiring above: `is_locked` (runs on every packet, before Gate 0) stays a pure local read with zero network calls in any configuration; `record_error` (the rare error path) uses the new `StateBackend::incr_with_ttl` to atomically bump a shared fleet-wide error counter and, on trip, a shared lockout marker — real cross-node circuit-breaker enforcement, not just eventual visibility. A background poller mirrors any fleet-wide lockout into the local map on a bounded interval (`RATE_LIMITER_DEFAULT_POLL_INTERVAL`, 2s default). A backend error fails *safe*, not open — `record_error` falls back to the original local-only algorithm for that call. `record_cover_traffic` stays node-local even with a backend configured (10x higher frequency, lower-severity consequence — see the code comment for the full tradeoff). `RRBCGateway` remains unwired — it has no production call site in `handler.rs` today, and its replay-safety semantics need a true atomic claim, not this write-behind pattern.
- `cscs.rs::CSCSLoopDetector` — deliberately the *lighter* of the two patterns above: `cs_detect_loop` runs on essentially every non-exempt packet, so only the rare **trip** event (never per-packet fingerprint recording) is mirrored to the backend, via a non-blocking bounded channel (`CSCS_MIRROR_QUEUE_CAPACITY`) — zero cost on the common path, best-effort forensic/dashboard visibility only, never consulted by `cs_detect_loop`'s own decision. A session bouncing between two nodes that each individually stay under threshold is a known, deliberately unaddressed residual gap (see `state_backend.rs`'s module doc).
...
- `gateway.rs::RRBCGateway` — its replay registry needs a true atomic claim (`SET NX EX`) for cross-node replay-safety, not read-through caching or write-behind mirroring; also has no production call site in `handler.rs` today, so this was correctly out of scope for the rate-limiter/CSCS hardening pass above.
...
| `gateway.rs` | `ZeroTrustGateway`, `AgentRateLimiter` (global singleton, `with_backend` for distributed cross-node circuit breaker), `RRBCGateway` | `gateway.py` |
| `aegf.rs` | `AEGFGovernor`, `ExecutionStateMachine`, `DistributedExecutionGraph` | `aegf.py` |
| `cscs.rs` | `CSCSLoopDetector` — oscillation + DAEG cycle detection, `with_backend` for best-effort cross-node trip mirroring | `cscs.py` |
